...
CORE_ROOT = Path(__file__).parents[2] / "src" / "extendcodeagent" / "core"
        ...
        for node in ast.walk(tree):
            ...
            for module in modules:
                if module.startswith(FORBIDDEN_IMPORT_PREFIXES):
                    violations.append(
                        f"{path.relative_to(CORE_ROOT)}:{getattr(node, 'lineno', 0)}: {module}"
                    )
    assert violations == []
...
def test_core_source_contains_no_atlas_or_opencode_contract_names() -> None:
    ...
    for path in sorted(CORE_ROOT.rglob("*.py")):
        ...
        for token in forbidden:
            if token in source:
                violations.append(f"{path.relative_to(CORE_ROOT)}: {token}")
    assert violations == []
